<?php
	...
	$sql = "SELECT venta_titulo, venta_descripcion FROM venta WHERE venta_id = '$empresa_id'";

	$lista = $conexion->query($sql);
	$resultado = mysqli_fetch_array($lista);

	$locales = explode(",", $resultado["venta_descripcion"]);
	
	$detalle = '<h3 style="color: rgb(68, 63, 63);">'.$resultado["venta_titulo"].'</h3><ul>';
